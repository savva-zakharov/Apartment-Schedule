...
'    Dim blockDict As Object
    ...
    If headerMap.Exists("TYPE") Then
        wsLong.columns(headerMap("TYPE")).NumberFormat = "@"
        wsShort.columns(headerMap("TYPE")).NumberFormat = "@"
        wsTypes.columns(headerMap("TYPE")).NumberFormat = "@"
        wsBlocks.columns(headerMap("TYPE")).NumberFormat = "@"
    End If

    ' Find the last used row in column C of the new sheet
    lastRow = wsLong.Cells(wsLong.rows.Count, 3).End(xlUp).row

    ' Delete rows where column C equals 0
    For i = lastRow To 2 Step -1 ' Start from the bottom to avoid skipping rows
        If wsLong.Cells(i, 4).Value = 0 Then
    ...
    For i = lastRow To 2 Step -1
        If wsLong.Cells(i, 6).Value = "XX" Or wsLong.Cells(i, 7).Value = "XX" Then
            wsLong.rows(i).Delete
        End If
    Next i
    ...
    With wsLong.Sort
        ...
        ' Apply the sorting to the range from column 1 to 15 (A to O)
        .SetRange wsLong.Range(wsLong.Cells(1, 1), wsLong.Cells(lastRow, 15))
    ...
    For i = 2 To lastRow
        Set rng = wsLong.Range(wsLong.Cells(i, 1), wsLong.Cells(i, 15))
    ...
    'find the last row
    lastRow = wsLong.Cells(wsLong.rows.Count, 5).End(xlUp).row
    ...
    For i = 2 To lastRow
        ' CStr so a type that still arrived as a number (an older source file,
        ' or a header the import could not match) is compared as text
        unitType = CStr(wsLong.Cells(i, 5).Value)
    ...
    For key = LBound(typeKeys) To UBound(typeKeys)
        ...
        wsTypes.Cells(outputRow, 5).Value = typeKeys(key)
    
        outputRow = outputRow + 1
    Next key


    wsTypes.Range(wsTypes.Cells(1, 2), wsTypes.Cells(wsTypes.rows.Count, 3)).ClearContents
    wsTypes.Range(wsTypes.Cells(1, 17), wsTypes.Cells(wsTypes.rows.Count, 19)).ClearContents
    wsTypes.Range(wsTypes.Cells(1, 1), wsTypes.Cells(wsTypes.rows.Count, 3)).Interior.ColorIndex = xlColorIndexNone

    lastRow = wsTypes.Cells(wsTypes.rows.Count, 5).End(xlUp).row
    With wsTypes.Sort
        .SortFields.Clear
        .SortFields.Add key:=wsTypes.Range(wsTypes.Cells(2, 5), wsTypes.Cells(lastRow, 5)), _
            SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal

        .SetRange wsTypes.Range(wsTypes.Cells(2, 1), wsTypes.Cells(lastRow, 14))
        .Header = xlNo
        .Apply
    End With

    Call drawBorderThickOutline(wsTypes.Range(wsTypes.Cells(2, 1), wsTypes.Cells(lastRow, 14)))

    wsTypes.Cells(lastRow + 1, 1).Formula = "=SUM(A2:A" & lastRow & ")"

    With wsTypes.columns(6).Font
        .Color = RGB(128, 128, 128) ' Grey text
        .Bold = True
    End With
    With wsTypes.columns(11).Font
        .Color = RGB(128, 128, 128) ' Grey text
        .Bold = True
    End With
    With wsTypes.columns(13).Font
        .Color = RGB(128, 128, 128) ' Grey text
        .Bold = True
    End With

    wsTemplate.rows(20).Resize(8).Copy
    wsTypes.Cells(1, 1).Insert Shift:=xlDown
